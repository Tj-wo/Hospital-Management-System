...
public class AdminAppointmentBean implements Serializable {
    ...
    public void init() {
        ...
        try {
            // Get the Role object for "DOCTOR" using roleService
            Role doctorRole = roleService.getRoleByName("DOCTOR");
            if (doctorRole != null) {
                availableDoctors = staffService.getStaffByRole(doctorRole);
            } else {
                ...
                availableDoctors = Collections.emptyList();
            }
        } catch (Exception e) {
            addMessage(FacesMessage.SEVERITY_ERROR, "Initialization Error", "Failed to load doctor roles: " + e.getMessage());
            availableDoctors = Collections.emptyList();
        }
    ...
    public void saveAppointment() {
        try {
            ...
            if (patient == null || doctor == null) {
                throw new ValidationException("Invalid patient or doctor selected.");
            }
            if (appointmentDate == null) {
                throw new ValidationException("Appointment date must be selected.");
            }
